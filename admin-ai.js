...
const AI = {
    ...
    calc: async (k, lock, limit, mxD, minPerDay) => {
        try {
            ...
            // ✅ BUGFIX: ร้านที่ AI ตัดทิ้งเป็นร้านโดด (ไม่ได้จัดวันให้เลย, days=[]) ยังมี dayOriginal
            // ("D02" ฯลฯ) ค้างจากไฟล์ import เดิม — FileManager._autoFillMarketNames() มี fallback
            // ไปใช้ dayOriginal เป็น key จัดกลุ่มตอนไม่มี days (ออกแบบไว้สำหรับตอน import ครั้งแรกที่
            // ยังไม่เคยจัดวันเลย) ทำให้ร้านโดดพวกนี้ถูกจัดกลุ่มปนไปกับเลข D เดิมที่ไม่เกี่ยวข้องกับ
            // การจัดสายรอบนี้อีกแล้ว ได้ชื่อตลาดหลอกๆ ที่ทำให้เข้าใจผิดว่ามีวันจัดจริง (เช่น
            // "403V01 D02 นราธิวาส" ทั้งที่ไม่มีวันเลย) ต้องเคลียร์ dayOriginal ทิ้งก่อนเสมอ
            // เพื่อให้ร้านที่ไม่มีวันจริงๆ ไม่ถูกจัดกลุ่ม (ปล่อยชื่อตลาดว่างไว้ให้เห็นชัดว่ายังไม่ได้จัด)
            State.stores.forEach(s => { if (!s.days || s.days.length === 0) s.dayOriginal = ''; });
