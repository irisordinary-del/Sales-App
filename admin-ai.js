const AI = {
    run: () => {
        ...
        UI.showLoader("AI กำลังวิเคราะห์พื้นที่...", "และจับกลุ่มร้านค้าที่อยู่ใกล้กัน"); 
        setTimeout(() => AI.calc(k, lock, limit, mxD), 100);
    },
    calc: (k, lock, limit, mxD) => {
        try {
            ...
            UI.hideLoader();
            alert("เกิดข้อผิดพลาดในการคำนวณ AI: " + err.message);
            console.error(err);
        }
    }
};
